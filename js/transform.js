...
Transform.prototype.update = function(width, height) {
    ...
    p.centerx = a.minx * (1.0 - a.alignx) + a.maxx * a.alignx;
    p.centery = a.miny * (1.0 - a.aligny) + a.maxy * a.aligny;

    p.sizex = c.scale * c.width;
    p.sizey = c.scale * c.height;

    p.minx = p.centerx - p.sizex * a.alignx;
    p.maxx = p.centerx + p.sizex * (1.0 - a.alignx);
    p.miny = p.centery - p.sizey * a.aligny;
    p.maxy = p.centery + p.sizey * (1.0 - a.aligny);
};
